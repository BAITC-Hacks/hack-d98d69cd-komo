...
export function GoalEditor({ development, busy, onSave, onClose }: { development: Development; busy: boolean; onSave: (goal: { target_role: string; target_grade: RoleOption['grade'] } | null) => Promise<void>; onClose: () => void }) {
  ...
  return <Panel><div className={s.heading}><h2>Выберите карьерную цель</h2><Button variant="secondary" onClick={onClose} disabled={busy}>Закрыть</Button></div>
    ...
    {!options.length && !error ? <Loading /> : <form onSubmit={e => { e.preventDefault(); if (selected) void onSave({ target_role: role, target_grade: grade }); }}>
      ...
      {selected && <details open className={s.preview}><summary>Требования выбранной цели</summary><ul>{Object.entries(selected.required_skills).map(([id, level]) => <li key={id}>{selected.skill_names[id]}: уровень {level}{selected.critical_skills.includes(id) ? ' · основной навык для этой роли' : ''}</li>)}</ul></details>}
      <div className={s.actions}><Button disabled={busy || !selected}>{busy ? 'Сохраняем…' : 'Сохранить цель'}</Button><Button type="button" variant="secondary" disabled={busy} onClick={() => void onSave(null)}>Предложить цель по текущей роли</Button></div>
    </form>}
  </Panel>;
}

...
export function EmployeeWorkspace({ employeeId }: { employeeId?: string }) {
  ...
  async function saveGoal(goal: { target_role: string; target_grade: RoleOption['grade'] } | null) {
    ...
    try {
      ...
      changed(dev); setEditingGoal(false); setSuccess('Цель сохранена. Теперь показываем, какие навыки нужны для нее. Ваш текущий уровень не изменился.');
      void recommend();
  ...
  async function mutate(action: 'start' | 'complete' | 'cancel', activity: ActivityOption, sessionDate?: string) {
    ...
    try {
      ...
      if ('gains' in result) {
        setSuccess(`Готово! ${result.gains.length ? result.gains.map(g => `${g.name}: ${g.before} → ${g.after}`).join(' · ') : 'Активность завершена. Ваш уровень навыков уже выше того, которому она учит'}. Соответствие цели: ${result.before_progress}% → ${result.development.progress}%.`);
        setSelected(null);
      } else if (action === 'cancel') { setSuccess('Участие отменено. Навыки не изменились.'); setSelected(null); }
      else {
        setSuccess(result.status === 'planned' ? 'Активность добавлена в ваш план. На само мероприятие нужно записаться отдельно.' : 'Активность начата. Навыки изменятся после подтверждения выполнения.');
  ...
  return <>
    ...
    <Recommendations result={recommendation} loading={thinking} readOnly={readOnly} completing={busy ? selected?.event_id || 'busy' : null} onOpen={open} currentProgress={development.progress} />
    {!!development.uncovered_critical_skills?.length && <div className={s.catalogNote}><strong>Что еще нужно для вашей цели</strong><p>Вам также нужно развить: {development.uncovered_critical_skills.join(', ')}. Сейчас в каталоге нет доступных вам активностей, которые напрямую повышают эти навыки.</p></div>}
    <div className={s.tabs}>{[['skills', 'Навыки для вашей цели'], ['participations', `Мои активности (${participations.length})`], ['history', `История участия (${history.length})`], ['activities', 'Подходящие активности']].map(([value, label]) => <button key={value} className={`${s.tab} ${tab === value ? s.selected : ''}`} onClick={() => setTab(value)}>{label}</button>)}</div>
